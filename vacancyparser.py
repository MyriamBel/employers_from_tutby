...
def getemployers(base_url, url, name_class, list_re):  # Получаем строку ссылки, строку имени класса тега, строку регулярки
    ...
    print(next_link)
    if len(next_link) > 0:
        getemployers(base_url=base_url, url=base_url + next_link[0], name_class=name_class,
...
getemployers(base_url=base_url, url=start_url, name_class=name_class, list_re=list_re)
